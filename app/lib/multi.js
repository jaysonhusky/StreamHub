...
function chat(e){
	$('.tabsforchat').removeClass("ui-tabs-active ui-state-active")
	$('#tab_'+e).addClass("ui-tabs-active ui-state-active");
	$('.chatcanvas').hide();
	$('#chat-'+e).show();
}
function AssociateStreamers(){
	var streamers = $('#streamname').val();	
	var cstream=streamers.replace(/\s/g,'');
	localStorage.removeItem('FFLMC'); // remove selected streamers
	localStorage.setItem('FFLMC', cstream); // Save to storage (for a better looking URL, strips whitespace to fix potential URL conflicts)
}